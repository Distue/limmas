...
suppressPackageStartupMessages(library(limmas))
```
...
| ------------- |------------- | 
| 1. Intensity Matrix    | MaxQuants proteinGroups.txt | 
| 2. Feature Data (Additional info to the features)  |  |
...
##### Protein intensity data from proteinGroups.txt 
...
With \Rfunction{hist} we can create a histogram of the non-missing values before the transformation and normalisation.
...
```
 ...
 as well as the transformed but not normalized data:
...
*m* imputed data sets into *ExpressionSets* and create an 
...
This approach is \textbf{not} desirable, because although many data points 
...
explain the analysis of the whole data set using \textbf{all} available data.
...
\emph{Note: Because of the missing values, `r Biocpkg("limma")` can return 
...
We are interested in improving the package, making the analysis of mass spectrometry data sets easy and especially well documented. Please send comments, feedback, and suggestions, or maybe a thanks to \texttt{schwarzl@embl.de}.
