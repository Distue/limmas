...
transformation and normalisation (Figure \ref{figure/histBeforeQC}).
...
The above functions can be employed by passing them into the \Rfunction{normalizeData} function call with the parameter. Remember that you can use the full functionality of an \Rclass{ExpressionSet} object. 
...
\RClass{ExpressionSet} has very useful functionality: e.g. we can plot histograms and boxplots of the normalized data
...
<<plotNAs, fig.width=5, fig.height=5>>=
   plotMedianVsNAs(data.transformed, "Gefh1", groupCol = "groups")
@

\incfig{figure/plotNAs}{0.6\textwidth}{plotNAs}{
}

This plot shows the standard deviation veersus the median expression

<<plotMedianVsSD, fig.width=5, fig.height=5>>=
   plotMedianVsSD(data.transformed, "Gefh1", groupCol = "groups")
@

\incfig{figure/plotMedianVsSD}{0.6\textwidth}{plotMedianVsSD}{
}

This plot shows the standard deviation versus the amount of missing values

<<plotNAsVsSD, fig.width=5, fig.height=5>>=
   plotNAsVsSD(data.transformed, "Gefh1", groupCol = "groups")
@

\incfig{figure/plotNAsVsSD}{0.6\textwidth}{plotNAsVsSD}{
}
...
%-- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- 
\subsection{Statistics for whole data set} 
%-- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- 

The analysis is done in two steps. First, we will use all features 
for building the linear model in \Rpackage{limma} while ignoring missing
...
@
\emph{Note: Because of the missing values, \Rpackage{limma} can return 
...
We are interested in improving the package, making
the analysis of mass spectrometry data sets easy and especially well documented. Please send comments, feedback, and suggestions,
